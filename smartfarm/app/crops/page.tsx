...
import AddEditCropDialog from "../../components/crops/AddEditCropDialog";
...
export default function CropsPage() {
  ...
  async function loadData() {
    setLoading(true);
    try {
      const fData = await dbService.getFields();
      setFields(fData);
    } catch (e) {
  ...
  return (
    <DashboardLayout>
          ...
          <header className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
            <div>
              ...
              <p className="text-sm font-medium text-slate-500 dark:text-zinc-400 mt-1">
                Monitor fields and crop cycles.
              </p>
            </div>
      ...
      <AddEditCropDialog open={fieldDialogOpen} onOpenChange={setFieldDialogOpen} field={selectedField} onSave={handleSaveField} />
    </DashboardLayout>
  );
}
